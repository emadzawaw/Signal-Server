...
void free_elev(void) { delete[] elev; }
...
void alloc_elev(void) { elev = new double[ARRAYSIZE + 10]; }
  ...
  for (x = 1; x <= y; x++) {
    ...
    if (strcmp(argv[x], "-o") == 0) {
      ...
      if (z <= y && argv[z][0] && argv[z][0] != '-') {
        ...
        if ((az_filename = (char *)calloc(strlen(argv[z]) + strlen(AZ_FILE_SUFFIX) + 1, sizeof(char))) == NULL) return ENOMEM;
        if (antenna_file[0] != '\0')
