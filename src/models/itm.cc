...
double fht(const double &x, const double &pk)
{
  ...
  if (x < 200.0) {
    ...
    if (pk < 1e-5 || (x * w * w * w) > 5495.0) {
      ...
      if (x > 1.0) fhtv = 17.372 * log(x) + fhtv;
    }
...
double ascat(double d, prop_type &prop, propa_type &propa)
{
  ...
  if (d == 0.0) {
    ...
    if (h0s > 15.0)
      ...
      if (et < 1.0)
        h0 = et * h0 +
             (1.0 - et) * 4.343 * log(pow((1.0 + 1.4142 / r1) * (1.0 + 1.4142 / r2), 2.0) * (r1 + r2) / (r1 + r2 + 2.8284));
...
void qlra(int kst[], int klimx, int mdvarx, prop_type &prop, propv_type &propv)
{
  ...
  for (int j = 0; j < 2; ++j) {
    if (kst[j] <= 0)
      ...
      if (prop.hg[j] < 5.0) q *= sin(0.3141593 * prop.hg[j]);
  ...
  if (prop.dist <= 0.0 || prop.dist >= propa.dlsa) {
    if (!wscat) {
      ...
      if (a5 < 1000.0) {
        ...
        propa.dx = mymax(propa.dlsa, mymax(propa.dla + 0.3 * xae * log(47.7 * prop.wn),
...
{
  static int kdv;
  static double dexa, de, vmd, vs0, sgl, sgtm, sgtp, sgtd, tgtd, gm, gp, cv1, cv2, yv1, yv2, yv3, csm1, csm2, ysm1, ysm2, ysm3,
      csp1, csp2, ysp1, ysp2, ysp3, csd1, zd, cfm1, cfm2, cfm3, cfp1, cfp2, cfp3;
  ...
  if (fabs(zt) > 3.1 || fabs(zl) > 3.1 || fabs(zc) > 3.1) prop.kwx = mymax(prop.kwx, 1);
...
void point_to_pointMDH(double tht_m, double rht_m, double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
...
void point_to_pointDH(double tht_m, double rht_m, double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
                      double frq_mhz, int radio_climate, int pol, double conf, double rel, double &dbloss, double &deltaH,
                      int &errnum)
{
  // pol: 0-Horizontal, 1-Vertical
  // radio_climate: 1-Equatorial, 2-Continental Subtropical, 3-Maritime Tropical,
...
void area(long ModVar, double deltaH, double tht_m, double rht_m, double dist_km, int TSiteCriteria, int RSiteCriteria,
          double eps_dielect, double sgm_conductivity, double eno_ns_surfref, double frq_mhz, int radio_climate, int pol,
...
double ITMAreadBLoss(long ModVar, double deltaH, double tht_m, double rht_m, double dist_km, int TSiteCriteria,
                     int RSiteCriteria, double eps_dielect, double sgm_conductivity, double eno_ns_surfref, double frq_mhz,
                     int radio_climate, int pol, double pctTime, double pctLoc, double pctConf)
{
  ...
  area(ModVar, deltaH, tht_m, rht_m, dist_km, TSiteCriteria, RSiteCriteria, eps_dielect, sgm_conductivity, eno_ns_surfref,
       frq_mhz, radio_climate, pol, pctTime, pctLoc, pctConf, dbloss, strmode, errnum);

  return dbloss;
}

double ITMVersion() { return 7.0; }
