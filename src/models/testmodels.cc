...
extern void point_to_point_ITM(double tht_m, double rht_m, double eps_dielect, double sgm_conductivity, double eno_ns_surfref,
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  fprintf(fh,
          "set terminal jpeg size 800,600\nset output '%.0fMHz_propagation_models.jpg'\nset title '%.0fMHz path loss by "
          "propagation model - HamWan'\n",
          f, f);
  fprintf(fh, "set key right bottom box\nset style line 1\nset grid\nset xlabel 'Distance KM'\nset ylabel 'Path Loss dB'\n");
  fprintf(fh,
          "plot 'FSPL' with lines, 'Hata.1' with lines, 'COST231.1' with lines,'ECC33.1' with lines,'Ericsson9999.1' with "
          "lines,'SUI.1' with lines,'Egli.VHF-UHF' with lines");
  fclose(fh);
